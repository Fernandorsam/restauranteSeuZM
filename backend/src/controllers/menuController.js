...
import { getCategories as _getCategories, getItems as _getItems,
getItemByIdOrSlug, searchItems as _searchItems, getPopularItems as _getPopularItems,
createItem as _createItem, updateItem as _updateItem, deleteItem as _deleteItem,
createCategory as _createCategory, updateCategory as _updateCategory,
 deleteCategory as _deleteCategory } from '../services/menuService.js';
import { success, paginated, notFound, error, created } from '../utils/ApiResponse.js';
...
class MenuController {
  ...
  getCategories = asyncHandler(async (req, res) => {
    ...
    return success(res, 'Categorias listadas com sucesso', { categories });
  });
  ...
  getItems = asyncHandler(async (req, res) => {
    ...
    const result = await _getItems(filters, {
      page: parseInt(page) || 1,
      limit: parseInt(limit) || 12
    });

    return paginated(res, 'Itens listados com sucesso', result);
  });
  ...
  getItem = asyncHandler(async (req, res) => {
    const item = await getItemByIdOrSlug(req.params.idOrSlug);
    if (!item) {
      return notFound(res, 'Item não encontrado');
    }
    return success(res, 'Item encontrado', { item });
  });
  ...
  searchItems = asyncHandler(async (req, res) => {
    ...
    if (!q) {
      return error(res, 'Termo de busca é obrigatório', 400);
    }

    const result = await _searchItems(q, {
      page: parseInt(page) || 1,
      limit: parseInt(limit) || 12
    });

    return paginated(res, 'Busca realizada com sucesso', result);
  });
  ...
  getPopularItems = asyncHandler(async (req, res) => {
    ...
    const items = await _getPopularItems(limit);
    return success(res, 'Itens populares', { items });
  });
  ...
  createItem = asyncHandler(async (req, res) => {
    const item = await _createItem(req.body);
    return created(res, 'Item criado com sucesso', { item });
  });
  ...
  updateItem = asyncHandler(async (req, res) => {
    const item = await _updateItem(req.params.id, req.body);
    return success(res, 'Item atualizado com sucesso', { item });
  });
  ...
  deleteItem = asyncHandler(async (req, res) => {
    await _deleteItem(req.params.id);
    return success(res, 'Item removido com sucesso');
  });
  ...
  createCategory = asyncHandler(async (req, res) => {
    ...
    return created(res, 'Categoria criada com sucesso', { category });
  });
  ...
  updateCategory = asyncHandler(async (req, res) => {
    ...
    return success(res, 'Categoria atualizada com sucesso', { category });
  });
