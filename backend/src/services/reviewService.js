...
import { findOne, create, find, countDocuments, aggregate,
findById, findByIdAndDelete } from '../models/review.js';
import { findById as _findById, findByIdAndUpdate } from '../models/menuItens.js';
import ApiError from '../utils/ApiError.js';
import { business, error as _error } from '../middlewares/logger.js';
import { notifyAdminNewReview } from './notificationService.js';
...
class ReviewService {
  ...
  async createReview(userId, data) {
    ...
    if (data.item) {
      const menuItem = await _findById(data.item);
      if (!menuItem) {
    ...
    if (data.item) {
      const existing = await findOne({ user: userId, item: data.item });
      if (existing) {
    ...
    await notifyAdminNewReview(review);
